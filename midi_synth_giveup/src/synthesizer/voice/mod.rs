...
impl Voice {
    pub(crate) fn new(settings: &SynthesizerSettings) -> Self {
        Self {
            ...
            exclusive_class: 0,
            channel: Channel::One,
            key: Key::from_databyte(0).unwrap(),
            velocity: 0,
            note_gain: 0_f32,
            cutoff: 0_f32,
            resonance: 0_f32,
    ...
    pub(crate) fn process(&mut self, data: &[i16], channels: &[ChannelProps]) -> bool {
        ...
        let pitch =
            self.key.byte() as f32 + vib_pitch_change + mod_pitch_change + channel_pitch_change;
    ...
    fn release_if_necessary(&mut self, channel_info: &ChannelProps) {

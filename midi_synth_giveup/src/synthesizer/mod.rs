...
use midix::prelude::{Channel, ChannelVoiceMessage, VoiceEvent};
use reverb::*;
...
pub struct Synthesizer {
    ...
    channels: FnvHashMap<Channel, ChannelProps>,
...
impl Synthesizer {
    ...
    pub const PERCUSSION_CHANNEL: Channel = Channel::Ten;
    ...
    pub fn new(
        ...
        let channels = Channel::all()
            ...
            .map(|channel| {
                (
                    channel,
                    ChannelProps::new(channel == Synthesizer::PERCUSSION_CHANNEL),
                )
            })
            .collect();
    ...
    fn get_channel(&self, channel: &Channel) -> &ChannelProps {
        // SAFETY:  Synthesizer can only be constructed with all Sixteen channels.
        unsafe { self.channels.get(&channel).unwrap_unchecked() }
    }

    pub fn process_voice_message(&mut self, event: ChannelVoiceMessage) {
        let ChannelVoiceMessage { channel, message } = event;
        match message {
            VoiceEvent::NoteOff {
                key,
                velocity: _velocity,
            } => {
                //TODO: handle velocity for note off
            ...
            VoiceEvent::NoteOn { key, velocity } => {
                ...
                let channel_info = self.get_channel(&channel);

                let preset_id =
                    (channel_info.get_bank_number() << 16) | channel_info.get_patch_number();
                    ...
                    if preset_region.contains(key, velocity) {
                        ...
                        for instrument_region in instrument.regions.iter() {
                            if instrument_region.contains(key, velocity) {
                                ...
                                if let Some(value) =
                                    self.voices.request_new(instrument_region, channel)
                                {

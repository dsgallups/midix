...
impl PresetRegion {
    ...
    pub fn contains(&self, key: Key, velocity: Velocity) -> bool {
        let key_byte = key.byte();
        let velocity_byte = velocity.byte();
        let contains_key =
            self.get_key_range_start() <= key_byte && key_byte <= self.get_key_range_end();

        let contains_velocity = self.get_velocity_range_start() <= velocity_byte
            && velocity_byte <= self.get_velocity_range_end();

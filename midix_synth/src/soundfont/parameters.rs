...
impl SoundFontParameters {
    pub(crate) fn new<R: Read + ?Sized>(reader: &mut R) -> Result<Self, SoundFontError> {
        ...
        let preset_infos = preset_infos.ok_or(SoundFontError::SubChunkNotFound(*b"PHDR"))?;

        let preset_bag = preset_bag.ok_or(SoundFontError::SubChunkNotFound(*b"PBAG"))?;

        let preset_generators =
            preset_generators.ok_or(SoundFontError::SubChunkNotFound(*b"PGEN"))?;

        let instrument_infos =
            instrument_infos.ok_or(SoundFontError::SubChunkNotFound(*b"INST"))?;

        let instrument_bag = instrument_bag.ok_or(SoundFontError::SubChunkNotFound(*b"IBAG"))?;

        let instrument_generators =
            instrument_generators.ok_or(SoundFontError::SubChunkNotFound(*b"IGEN"))?;

        let sample_headers = sample_headers.ok_or(SoundFontError::SubChunkNotFound(*b"SHDR"))?;
